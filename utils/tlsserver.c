...
static int8_t tls13_load_server_certificate_and_key(tls13_context_t* tls13_ctx) {
    ...
    if(x509_certificate_verify_signature(ca_certificate, ca_public_key, sizeof(ca_public_key)) != 0) {
    ...
    uint8_t* server_private_key = memory_malloc(ED25519_PRIVATE_KEY_RAW_LEN);
    ...
    if(tls13_set_server_certificate(tls13_ctx, cert, server_private_key, ED25519_PRIVATE_KEY_RAW_LEN) != 0) {
