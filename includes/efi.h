 ...
 */
typedef struct efi_pmbr_partition_t {
 ...
 */
typedef struct efi_table_header_t {
...
typedef struct efi_partition_table_header_t {
...
typedef struct efi_partition_entry_t {
...
typedef enum efi_allocate_type_t {
...
typedef enum efi_locate_search_type_t {
    EFI_ALL_HANDLES,
    EFI_BY_REGISTER_NOTIFY,
    EFI_BY_PROTOCOL,
} efi_locate_search_type_t;

typedef enum efi_reset_type_t {
    EFI_RESET_COLD,
    EFI_RESET_WARM,
    EFI_RESET_SHUTDOWN,
} efi_reset_type_t;
...
typedef enum efi_timer_delay_t {
    EFI_TIMER_CANCEL,
    EFI_TIMER_PERIODIC,
    EFI_TIMER_RELATIVE,
    EFI_TIMER_TYPE_MAX,
...
typedef enum efi_device_path_type_t {
...
typedef enum efi_device_path_subtype_hardware_t {
...
typedef enum efi_device_path_subtype_acpi_t {
...
typedef enum efi_device_path_subtype_media_t {
...
typedef struct efi_device_path_t {
...
typedef struct efi_input_key_t {
    uint16_t scan_code;
...
typedef struct efi_simple_input_interface_t {
...
typedef struct efi_simple_output_mode_t {
    int32_t   max_mode;
    int32_t   mode;
    int32_t   attribute;
...
typedef struct efi_simple_text_output_interface_t {
...
typedef struct efi_capsule_header_t {
...
typedef struct efi_capsule_block_descriptor_t {
    uint64_t               length;
...
typedef struct efi_open_protocol_information_entry_t {
...
typedef struct efi_system_table_t {
    ...
    efi_simple_input_interface_t* console_input;

    efi_handle_t                        console_output_handle;
    efi_simple_text_output_interface_t* console_output;

    efi_handle_t                        console_error_handle;
    efi_simple_text_output_interface_t* console_error;
...
typedef struct efi_block_io_media_t {
    uint32_t  media_id;
...
typedef enum efi_gop_pixel_format_t {
...
typedef enum efi_gop_blt_operation_t {
    EFI_BLT_VIDEO_FILL,
...
typedef struct efi_gop_pixel_bitmask_t {
    uint32_t red_mask;
    uint32_t green_mask;
    uint32_t blue_mask;
...
typedef struct efi_gop_mode_info_t {
    uint32_t                version;
...
typedef struct efi_loaded_image_t {
    uint32_t            revision;
...
typedef struct efi_network_ipv4_address_t {
    uint8_t addr[4];
}efi_network_ipv4_address_t;

typedef struct efi_ipv6_address_t {
    uint8_t addr[16];
}efi_ipv6_address_t;

typedef union efi_ip_address_t {
    uint32_t                   addr[4];
    efi_network_ipv4_address_t v4;
    efi_ipv6_address_t         v6;
} efi_ip_address_t;

typedef struct efi_network_mac_address_t {
    uint8_t addr[32];
} efi_network_mac_address_t;

typedef enum efi_pxe_base_code_ip_filter_type_t {
...
typedef struct efi_pxe_base_code_ip_filter_t {
...
typedef struct efi_pxe_base_code_icmp_error_t {
    uint8_t  type;
    uint8_t  code;
    uint16_t checksum;
    union {
...
typedef struct efi_pxe_base_code_dhcpv4_packet_t {
...
typedef struct efi_pxe_base_code_dhcpv6_packet_t {
...
typedef union efi_pxe_base_code_packet_t {
    uint8_t                           raw[1472];
...
typedef struct efi_base_code_mode_t {
    boolean_t                       started;
...
typedef enum efi_pxe_base_code_boot_type_t {
...
typedef struct efi_pxe_base_code_srvlist_t {
    uint16_t         type;
...
typedef struct efi_pxe_base_code_discover_info_t {
    boolean_t                   use_mcast;
    boolean_t                   use_bcast;
    boolean_t                   use_ucast;
...
typedef enum efi_pxe_base_code_tftp_opcode_t {
...
typedef struct efi_pxe_base_code_mtftp_info_t {
...
typedef struct efi_pxe_base_code_protocol_t {
    uint64_t                       revision;
...
typedef struct efi_image_header_t {
    uint32_t magic;
    uint16_t machine;
...
typedef struct efi_image_optinal_data_table_t {
    uint32_t virtual_address;
    uint32_t size;
} __attribute__((packed)) efi_image_optinal_data_table_t;

typedef struct efi_image_optional_header_t {
    uint16_t                       magic;
...
typedef struct efi_image_section_header_t {
    char_t   name[8];
...
typedef struct efi_image_relocation_entry_t {
    uint32_t page_rva;
    uint32_t block_size;
    struct {
        uint16_t offset : 12;
