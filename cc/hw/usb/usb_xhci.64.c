...
static int8_t usb_xhci_pool_event(usb_controller_t*   usb_controller,
    ...
    while(timeout--) {
        ...
        if((event_trb->control & 1) == 1 && event_trb->parameter == target_trb_fa) {
        ...
        event_index = (event_index + 1) % USB_XHCI_EVENT_RING_SIZE;
        time_timer_spinsleep(100);
    }
...
static int8_t usb_xhci_set_slot_and_address(usb_controller_t* usb_controller, usb_transfer_t* transfer){
    ...
    ep0_ctx[2] = (dcb_fa + 0x400 + 8 * sizeof(uint32_t)) | 1;
    ep0_ctx[3] = 0;
    ep0_ctx[4] = 8;
...
static int8_t usb_xhci_control_transfer(usb_controller_t* usb_controller, usb_transfer_t* transfer) {
    ...
    uint64_t* dcbaa = metadata->dcbaa;
    uint64_t dcb_fa = dcbaa[device->slot_id];
    uint64_t dcb_va = MEMORY_PAGING_GET_VA_FOR_RESERVED_FA(dcb_fa);

    uint64_t transfer_ring_fa = dcb_fa + 0x400 + 8 * sizeof(uint32_t);
    uint64_t transfer_ring_va = dcb_va + 0x400 + 8 * sizeof(uint32_t);
    ...
    PRINTLOG(USB, LOG_TRACE, "XHCI control transfer for slot %d, dev_ctx=0x%llx, transfer_ring=0x%llx",
             device->slot_id, dcb_fa, transfer_ring_fa);
...
int8_t usb_xhci_init(usb_controller_t* usb_controller) {
    ...
    PRINTLOG(USB, LOG_DEBUG, "XHCI Event Ring initialized at 0x%016llx", (uint64_t)event_ring);
