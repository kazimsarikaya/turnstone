...
int8_t usb_device_init(usb_device_t* parent, usb_controller_t* controller, uint32_t port, uint32_t speed) {
    ...
    for(int32_t i = 0; i < device_desc.num_configurations; i++) {
        ...
        while(idx < total_length) {
            ...
            if(type == USB_BASE_DESC_TYPE_INTERFACE) {
                ...
                PRINTLOG(USB, LOG_TRACE, "hub descriptor found");
                config->hub = (usb_hub_desc_t*)(config->config_buffer + idx);
            } else if(type == USB_CLASS_DESC_TYPE_CS_INTERFACE) {
                if(ep_idx == 0) {
                    PRINTLOG(USB, LOG_ERROR, "no endpoint for cs interface descriptor");
                    usb_device_free(usb_device);

                    return -1;
                }
                if(!config->endpoints) {
                    PRINTLOG(USB, LOG_ERROR, "no endpoints for cs interface descriptor");
                    usb_device_free(usb_device);

                    return -1;
                }
                if(!config->endpoints[ep_idx - 1]) {
                    PRINTLOG(USB, LOG_ERROR, "no endpoint for cs interface descriptor");
                    usb_device_free(usb_device);
                ...
                if(!config->endpoints) {
                    PRINTLOG(USB, LOG_ERROR, "no endpoints for endpoint companion descriptor");
                    usb_device_free(usb_device);
                ...
                PRINTLOG(USB, LOG_TRACE, "endpoint companion descriptor for endpoint index 0x%x",
                         config->endpoints[ep_idx - 1]->desc->endpoint_address);
                config->endpoints[ep_idx - 1]->endpoint_companion = (usb_endpoint_companion_desc_t*)(config->config_buffer + idx);
            }else {
    ...
    int32_t interface_class = selected_config->interface->interface_class;
    int32_t interface_subclass = selected_config->interface->interface_subclass;
    int32_t interface_protocol = selected_config->interface->interface_protocol;
    int32_t num_endpoints = selected_config->interface->num_endpoints;
    ...
    usb_endpoint_t** endpoints = selected_config->endpoints;

    for(int32_t i = 0; i < num_endpoints; i++) {
        uint32_t ep_address = endpoints[i]->desc->endpoint_address;
        uint32_t ep_max_packet_size = endpoints[i]->desc->max_packet_size;
