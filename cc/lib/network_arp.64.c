...
extern ipv4_address_t NETWORK_TEST_OUR_STATIC_IP;

uint8_t* network_arp_create_reply_from_packet(network_arp_t* src_arp_packet, mac_address_t mac, uint16_t* return_packet_len);
...
uint8_t* network_arp_create_reply_from_packet(network_arp_t* src_arp_packet, mac_address_t mac, uint16_t* return_packet_len) {
    ...
    memory_memcopy(src_arp_packet->source_ip, arp_packet->target_ip, sizeof(ipv4_address_t));
    memory_memcopy(src_arp_packet->source_mac, arp_packet->target_mac, sizeof(mac_address_t));
    ...
    memory_memcopy(mac, arp_packet->source_mac, sizeof(mac_address_t));
    memory_memcopy(src_arp_packet->target_ip, arp_packet->source_ip, sizeof(ipv4_address_t));
...
network_arp_t* network_arp_create_request(mac_address_t src_mac, ipv4_address_t src_ip, ipv4_address_t tgt_ip){
    network_arp_t* arp_packet = memory_malloc(sizeof(network_arp_t));
