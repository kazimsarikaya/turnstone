...
static int8_t linkedlist_narrow(list_t* list, size_t s, const void* data, list_item_t** head, list_item_t** tail, size_t* position) {
    list_item_t* h = *head;
    list_item_t* t = *tail;
...
const void* linkedlist_delete_at(list_t* list, const void* data, list_insert_delete_at_t where, size_t position){
    ...
    if(where == LIST_DELETE_AT_HEAD) {
        ...
        if(list->type == LIST_TYPE_INDEXEDLIST) {
            ...
            if(!pk_iter->end_of_iterator(pk_iter)) {
...
static int8_t linkedlist_iterator_destroy(iterator_t* iterator) {
    if(iterator == NULL) {
        return -1;
    }
...
static iterator_t* linkedlist_iterator_next(iterator_t* iterator) {
    if(iterator == NULL) {
        return NULL;
    }
...
static boolean_t linkedlist_iterator_end_of_list(iterator_t* iterator) {
    linkedlist_iterator_internal_t* iter = (linkedlist_iterator_internal_t*)iterator->metadata;
    return iter->current == NULL;
}

static const void* linkedlist_iterator_get_item(iterator_t* iterator) {
    list_item_t* item = ((linkedlist_iterator_internal_t*)iterator->metadata)->current;
    return item->data;
}

static const void* linkedlist_iterator_delete_item(iterator_t* iterator){
